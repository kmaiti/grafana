...
ENV PATH=/usr/share/grafana/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
    GF_PATHS_CONFIGDIR="/etc/grafana" \
...
RUN apt-get update && apt-get install -qq -y tar libfontconfig curl ca-certificates && \
    ...
    mkdir -p "$GF_PATHS_PROVISIONING/datasources" \
             ...
             "$GF_PATHS_CONFIGDIR" \
             "$GF_PATHS_DATA" && \
    touch "$GF_PATHS_CONFIG" && \
    cp "$GF_PATHS_HOME/conf/sample.ini" "$GF_PATHS_CONFIG" && \
    cp "$GF_PATHS_HOME/conf/ldap.toml" /etc/grafana/ldap.toml && \
    chown -R grafana:grafana "$GF_PATHS_DATA" "$GF_PATHS_CONFIG" "$GF_PATHS_HOME/.aws" "$GF_PATHS_LOGS" "$GF_PATHS_PLUGINS" "$GF_PATHS_CONFIGDIR" && \
